...
# Match the rest of chapter 3's figures: light gray for plumbing stages,
# pale blue for the focal "classifier" stage, pale orange for the final
# decision (echoes the projection color in tcn_backbone.py).
...
def render(out_name: str) -> Path:
    ...
    # Block widths/heights — pipeline is horizontal, so wide-and-short blocks.
    block_h = 0.36
    centers = [0.10, 0.30, 0.52, 0.74, 0.92]
    widths  = [0.13, 0.17, 0.17, 0.17, 0.13]

    # Audio input — neutral box, with a small stylised waveform sitting at
    # the subtitle level (matches where the other blocks' subtitles render).
    ...
    # Classifier — the focal stage, pale blue.
    clf = box(centers[3], y_main, widths[3], block_h,
              "Classifier", CLASSIFIER_COLOR, fontsize=12)
    subtitle(centers[3], "GMM, CNN, ...")

    # Decision — pale orange, slightly emphasized.
    dec = box(centers[4], y_main, widths[4], block_h,
              "Decision", DECISION_COLOR, fontsize=12)
    subtitle(centers[4], "Speech/Music")

    # Inter-block arrows. Terminate just outside each box edge so the arrow-
    # head is fully visible (shrinkA/shrinkB on top of geometric edge-to-edge).
