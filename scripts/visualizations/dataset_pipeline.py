...
def render(out_name: str) -> Path:
    ...
    # Match the boxstyle pad used in node() so arrow/feeder endpoints land on
    # the visible box edge rather than the geometric one (the rounded box's
    # visible edge sits a `pad` unit outside the geometric edge).
    BOX_PAD = 0.004
    ...
    # Stage 1: sources -> labelers -> real subclasses
    src_x = 0.06
    ...
    # Stage 2: augmentations.
    # Augment-block y's are chosen so that real-subclass feeder lines (drawn
    # below) at y in {0.80, 0.68, 0.54, 0.42, 0.28} fall cleanly into the
    # gaps between augment blocks rather than passing through them.
    aug_x = 0.61
    ...
    # The 1-to-many input relationships (clean speech feeds every augmentation,
    # music feeds two, noise feeds one) are not drawn as per-input arrows —
    # they produce an unreadable X-cross. The recipes are spelled out in each
    # block's caption and in the chapter prose; the gather bus below is what
    # visually ties everything to stage 3.

    # Gather bus: a vertical line just right of the augment column. Both real
    # subclasses and augmentations feed into the bus. Real-subclass feeders
    # are thin and faded; they pass cleanly between augment blocks because
    # of the y-position choice above. A single arrow exits the bus midpoint
    # into the Dataset container.
    bus_x = 0.715
    bus_top = 0.83
    bus_bot = 0.25
    # Feeders are drawn first; bus is drawn afterwards on top so the bus
    # cleanly covers each feeder's terminal pixel. Feeders end exactly at
    # bus_x — no overshoot — so they do not protrude past the bus on the
    # right.
    for r in real_nodes:
        ax.plot(
    ...
    # Stage 3: a Splitter block performs the per-source per-subclass split
    # into train/val/test and feeds the Dataset container.
    split_x = 0.81
    split = node(
    ...
    sub_h = 0.07
    sub_gap = 0.005
    sub_top = dataset_top - 0.06  # below the header
    parts = [
        ("train (84%)", "#C5E1A5"),
        ("val (8%)",    "#FFE082"),
        ("test (8%)",   "#EF9A9A"),
    ...
    # Bus -> Splitter -> Dataset. The "all 14 subclasses" label sits just
    # under the top of the bus.
    arrow(
        (bus_x, split[1]),
