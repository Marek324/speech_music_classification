...
from _common import REPO_ROOT, setup_style  # noqa: F401

# Override the default OUT_DIR so dataset figures live alongside chapter 4.
...
def draw_subpie(ax_pie, ax_legend, sub: dict, base_color: str, title: str):
    ...
    # Mask the center where all wedge edges meet (otherwise overlapping
    # strokes create a visible star artifact for narrow wedges).
...
def render(out_name: str) -> Path:
    ...
    # Top row: 3 pies. Bottom row: 2 legends (under speech / music subpies).
    gs = fig.add_gridspec(
        2, 3,
    ...
    # Trapezoidal connectors: top edge of source slice → top of subpie,
    # bottom edge of source slice → bottom of subpie. shrinkA/shrinkB add a
    # small gap (in points) at each end so lines don't touch the pies.
    GAP_MAIN_PT     = 7    # main-pie side (both top and bottom)
    GAP_SUB_TOP_PT  = 17   # subpie side, top line (clears the title)
    GAP_SUB_BOT_PT  = 12   # subpie side, bottom line
    SUB_Y_OFFSET_TOP_PT  = 2   # subpie-side top endpoint nudged +2pt up
    SUB_Y_OFFSET_BOT_PT  = 1   # subpie-side bottom endpoint nudged -1pt down
    MAIN_Y_OFFSET_BOT_PT = 1   # main-pie-side bottom endpoint nudged -1pt down
