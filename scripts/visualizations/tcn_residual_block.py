...
def render(out_name: str) -> Path:
    ...
    # Bottom-up flow with uniform centre-to-centre spacing within each group
    # of four (conv -> WN -> sigma -> dropout) and a wider gap between groups.
    # An extra sigma sits between the residual sum and the output, matching
    # the post-add ReLU used in the reference implementation.
    step = 0.080
    group_gap = 0.045  # extra spacing on top of `step` between drop1 and conv2
    y_input  = 0.030
    y_conv1  = 0.125  # extra headroom above the input arrow so the skip branch has its own breathing space
    y_wn1    = y_conv1 + step
    y_act1   = y_wn1   + step
    y_drop1  = y_act1  + step
    ...
    y_act2   = y_wn2   + step
    y_drop2  = y_act2  + step
    y_add     = y_drop2 + 0.085  # breathing room before the sum
    y_act_out = y_add    + 0.085  # post-add non-linearity
    ...
    # Skip path on the right: branch off near the input, run up the side
    # through a 1x1 projection, then enter the sum from the right.
    skip_x = 0.84
    # Branch from the middle of the input -> conv1 arrow.
    ...
    # Legend — sits in the left margin, vertically centred next to the
    # diagram (main path spans x in roughly [0.45, 0.75]).
    leg_x = 0.00
    swatch_w, swatch_h = 0.040, 0.030
    label_fs = 10
    line_step = 0.024  # vertical step between consecutive lines within an entry
    entry_gap = 0.040  # gap between the two legend entries
    ...
    # Compute total legend height so we can centre it vertically.
    entry_heights = [
    ...
    cursor = 0.5 + total_h / 2  # top of first swatch
