...
def main():
    ...
    jazz = load_dataset("PlutoG99001/MusicGen-Jazz-Clean", split="train", streaming=True)
    jazz = jazz.shuffle(seed=RAND_SEED).select_columns("audio").cast_column("audio", Audio(sampling_rate=SR, num_channels=1, decode=False))
    process_dataset(jazz, msd, "music", "music_acoustic", 50, "jazz", sil_thr=-20)

    print("Downloading country")
    country = load_dataset("ylacombe/music_genres_Country", split="train", streaming=True)
    country = country.shuffle(seed=RAND_SEED).select_columns("audio").cast_column("audio", Audio(sampling_rate=SR, num_channels=1, decode=False))
    process_dataset(country, msd, "music", "music_acoustic", 142, "country", sil_thr=-25)

    print("Downloading electronic")
    electronic = load_dataset("PlutoG99001/MusicGen-Electronic-Clean", split="train", streaming=True)
    electronic = electronic.shuffle(seed=RAND_SEED).select_columns("audio").cast_column("audio", Audio(sampling_rate=SR, num_channels=1, decode=False))
    process_dataset(electronic, msd, "music", "music_electronic", 52, "electronic", sil_thr=-25)

    print("Downloading instrumental")
    instrumental = load_dataset("PlutoG99001/MusicGen-Classical", split="train", streaming=True)
    instrumental = instrumental.shuffle(seed=RAND_SEED).select_columns("audio").cast_column("audio", Audio(sampling_rate=SR, num_channels=1, decode=False))
    process_dataset(instrumental, msd, "music", "music_instrumental", 495, "instrumental", sil_thr=-27)
