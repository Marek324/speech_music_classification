...
class DemoState(rx.State):
    ...
    # Mic-mode input selection.
    mic_device: int = -1                 # -1 = system default (PortAudio index)
    audio_source_mode: str = _SOURCE_MIC  # mic | mic_desktop | desktop
    desktop_source: str = ""             # Pulse/Pipewire monitor source name
    # Monitor sources tap the sink pre-analog-amp, so apps with low per-stream
    # volume (e.g. Firefox at 34%) sound loud in headphones but feed the model
    # at -30 dBFS. This slider compensates without touching pavucontrol.
    ...
    proc_mem_mb: float = 0.0       # process RSS in MB
