...
**Transformer scaling regresses.** `transformer_small` lands at 0.8935 — already 8 pp below baseline. `transformer_large` collapses further to 0.6942: the per-class breakdown shows the larger model dumps most background frames into music (recall on `background` falls from 0.69 to 0.20; `noise` subclass F1 from 0.87 to 0.13). At seq_len=128 the larger Transformer apparently overfits the speech/music boundary while losing the noise/silence axis entirely. Likely causes: (i) short seq_len starves attention of the long-range context it's designed to exploit; (ii) no positional prior matching the strong local temporal structure of speech/music frames; (iii) background is the rarest class and the most sensitive to under-regularization.
