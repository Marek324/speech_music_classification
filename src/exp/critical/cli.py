...
# Per-label palette: speech=blue, music=crimson, background=grey
...
# Map class label to y-axis position for trace plots.
_CLASS_Y = {-1: -1.0, 2: 0.0, 1: 1.0}  # speech=-1, background=0, music=+1
_MODEL_ORDER = ("tcn_l", "tcn", "tcn_s", "decision_tree", "svm", "gmm")

_NN_VARIANT_NAMES = {"tcn_s", "tcn_l"}  # populated lazily from VARIANTS
...
# ---------------------------------------------------------------------------
# Inference drivers — one per backend. Each writes ``{model}_scores.npz`` for
# the timeline visualizer and does NOT compute aggregate metrics.
# ---------------------------------------------------------------------------
...
def _draw_timeline_panel(
    ...
    # Background: GT as faded full-height vspans.
    ...
    # Foreground: per-model step traces.
