...
| Model | Macro F1 | CI | Speech F1 | Music F1 | Inactive F1 |
|---|---|---|---|---|---|
...
1. **Babble / machinery / outdoor noise**. `noise_babble`, `noise_machinery`, and `noise_outdoor` all land near zero F1 across every classifier (recall 0.0000–0.0577). These conditions are categorically absent from the train/val/test noise distribution (which is dominated by `noise_ambient` and `noise_wildlife`), and no model has learned to label them as inactive. GMM is the partial exception — `noise_ambient` recall 0.84, vs 0.39–0.65 for everything else — because GMM's inactive-vs-speech log-likelihood threshold happens to fire on lower-frequency hum.
...
The crit tier contains 8 switching clips: 2-class (speech↔music) and 3-class (speech↔music↔inactive) at 500/1000/2000/4000 ms cadences. Subclass F1 captures the static-frame view of these (e.g., `speech_switching_3class_500ms`); the *temporal* view (per-event latency, stable-region flicker) is computed by the transitions experiment from these same `_scores.npz` files — see `src/exp/transitions/results/transitions.md`.

## Conclusion

**TCN+LSTM is the clear winner** under adversarial conditions, +0.05 macro F1 over the next-best model and the only one with `inactive` F1 above 0.45. The LSTM tail's persistent state apparently smooths the decision in noisy / multi-talker frames in a way the pure-conv models cannot match.
...
**Classics (DT, SVM) trail** — DT by its inability to label inactive at all (F1 0.17), SVM by a uniform 0.04 F1 deficit on every class. GMM is unexpectedly strong on noise specifically (`noise_ambient` recall 0.84) which lifts its macro F1 above the smaller TCNs.
