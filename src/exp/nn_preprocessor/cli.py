...
@click.group("nn-preprocessor")
def nn_preprocessor_group():
    """Preprocessor layer experiment — variants defined in src/exp/nn_preprocessor/config.toml."""
    pass
...
def eval_cmd(name):
    """Evaluate a single preprocessor variant on the test split."""
...
@nn_preprocessor_group.command("smoke-test")
...
def run_all_cmd(skip_existing):
    """Train + eval all preprocessor variants sequentially."""
    import time
    ...
    log.info("Running preprocessor experiment: %d variant(s): %s", len(names), names)
    ...
    log.info("Preprocessor experiment complete. %d variant(s) processed.", len(done))
