...
@click.group("small-tcn-stacks")
def small_tcn_stacks_group():
    """SmallTCN stacks sweep — n_stacks (and thus receptive field) curve at the SmallTCN recipe."""
    pass


@small_tcn_stacks_group.command("train")
@click.option("--name", "-n", default=None, help="Variant name (e.g. stacks_1)")
@click.option("--no-wandb", is_flag=True, default=False, help="Disable W&B logging")
def train_cmd(name, no_wandb):
    """Train a single SmallTCN-stacks variant."""
...
@small_tcn_stacks_group.command("eval")
@click.option("--name", "-n", default=None, help="Variant name")
def eval_cmd(name):
    """Evaluate a single SmallTCN-stacks variant on the test split."""
...
@small_tcn_stacks_group.command("smoke-test")
...
def smoke_test_cmd(name):
    ...
    log.info(
        "Smoke-test passed. Variant: %s  n_stacks: %d  n_features: %d  Params: %d  Output shape: %s",
        variant_name, cfg["model"]["n_stacks"], nf, n_params, tuple(probs.shape),
    )


@small_tcn_stacks_group.command("visualize")
def visualize_cmd():
    """Plot SmallTCN-stacks sweep results. Reads results/tcn_<name>.eval files."""
    ...
    mod.main(config_path=_CFG_PATH, results_dir=_results_dir, experiment_name="SmallTCN Stacks Sweep")


@small_tcn_stacks_group.command("run-all")
...
def run_all_cmd(skip_existing):
    """Train + eval all SmallTCN-stacks variants sequentially."""
    import time
    ...
    log.info("Running SmallTCN-stacks experiment: %d variant(s): %s", len(names), names)
    ...
    log.info("SmallTCN-stacks experiment complete. %d variant(s) processed.", len(done))
