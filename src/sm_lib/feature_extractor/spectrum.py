...
def spectrum_rolloff_point(frame: np.ndarray, sr: int, n_fft: int, thr: float = 0.85) -> float:
    return float(ft.spectral_rolloff(y=frame, sr=sr, roll_percent=thr, n_fft=n_fft)[0, 0])
