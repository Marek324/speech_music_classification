...
def _load(name: str | None = None):
    ...
    if name:
        ...
        cfg = get_config(_CFG_PATH)
    variant_name = cfg.get("name", "architecture")
    return (
        cfg,
        variant_name,
...
@click.group("nn-architecture")
def nn_architecture_group():
    """Backbone architecture experiment — variants defined in src/exp/nn_architecture/config.toml."""
    pass
...
def eval_cmd(name):
    """Evaluate a single backbone variant on the test split."""
...
@nn_architecture_group.command("smoke-test")
...
def run_all_cmd(skip_existing):
    """Train + eval all backbone variants sequentially."""
    import time
    ...
    log.info("Running architecture experiment: %d variant(s): %s", len(names), names)
    ...
    log.info("Architecture experiment complete. %d variant(s) processed.", len(done))
