...
def _load(name: str | None = None):
    ...
    if name:
        ...
        cfg = get_config(_CFG_PATH)
    variant_name = cfg.get("name", "combined")
    return (
        cfg,
        variant_name,
...
@click.group("tcn-combined")
def tcn_combined_group():
    """TCN combined-winners experiment — variants defined in src/exp/tcn_combined/config.toml."""
    pass
...
def eval_cmd(name):
    """Evaluate a single combined-winners variant on the test split."""
...
@tcn_combined_group.command("smoke-test")
...
def run_all_cmd(skip_existing):
    """Train + eval all combined-winners variants sequentially."""
    import time
    ...
    log.info("Running combined-winners experiment: %d variant(s): %s", len(names), names)
    ...
    log.info("Combined-winners experiment complete. %d variant(s) processed.", len(done))
